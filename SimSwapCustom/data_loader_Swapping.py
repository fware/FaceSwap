...
class SwappingDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.dataset        = []
        self.num_images     = 0 # Initialize to 0, will be updated in _preprocess_dataset
        self._preprocess_dataset()

    def _preprocess_dataset(self):
        print("processing Swapping dataset images...")
        pathes = glob.glob(os.path.join(self.image_dir,'*'))
        for dir_item in pathes:
            join_path = glob.glob(os.path.join(dir_item,'*.jpg'))
            print("processing %s"%dir_item,end='\r')
            if join_path: # Only append if the directory contains images
                self.dataset.append(join_path)
        random.seed(self.random_seed)
        random.shuffle(self.dataset)
        self.num_images = len(self.dataset) # Update num_images to reflect actual directories with images
    ...
    def __getitem__(self, index):
        ...
        # Ensure dir_tmp1_len is not zero before calling random.randint
        if dir_tmp1_len == 0:
            # This case should ideally be avoided by the _preprocess_dataset fix,
            # but added as a safeguard or for debugging if an empty list still appears.
            raise ValueError(f"Attempted to sample from an empty directory list at index {index}")
